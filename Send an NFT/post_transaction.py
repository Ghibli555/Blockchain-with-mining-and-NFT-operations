...
def send_transaction(secret_key_sender_str, public_key_receiver_str, coin_num, nft_data, nft_origin):
    ...
    unsigned_transaction = {"time": time_now, "sender": public_key_sender_str, "receiver": public_key_receiver_str,
                            "amount": coin_num, "nft_data": nft_data, "nft_origin": nft_origin}
    signature = secret_key_sender.sign(json.dumps(unsigned_transaction).encode('utf-8'))
    transaction = {"time": time_now, "sender": public_key_sender_str, "receiver": public_key_receiver_str,
                   "amount": coin_num, "nft_data": nft_data, "nft_origin": nft_origin, "signature": signature.hex()}
