...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_node = {executor.submit(requests.get, "http://" + node + ":8000/chain"): node for node in
                          node_list.Node_List}
        max_chain_len = 0
            ...
            try:
                ...
                if blockchain.verify_chain(temp_dict):
                    if max_chain_len < len(temp_dict["blocks"]):
                        ...
                        temp_pow_difficulty = blockchain.current_pow_difficulty
    ...
    blockchain.current_pow_difficulty = temp_pow_difficulty
    ...
    blockchain.nft_calc(all_block_transactions_copy)
    transactions = all_block_transactions_copy[len(blockchain.all_block_transactions):]
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_node = {
            executor.submit(requests.post, "http://" + node + ":8000/chain", json.dumps(blockchain.chain)): node for
            node in node_list.Node_List}
        for future in concurrent.futures.as_completed(future_to_node):
            node = future_to_node[future]
            try:
                ...
                print("%s generated an exception: %s" % (node, exc))
    print("self.current_pow_difficulty = " + str(blockchain.current_pow_difficulty))