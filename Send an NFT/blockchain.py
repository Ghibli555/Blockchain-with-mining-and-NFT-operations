...
class BlockChain(object):
    ...
    def add_transaction_pool(self, transaction):
        if (transaction not in self.all_block_transactions) and (
                transaction not in self.transaction_pool["transactions"]):
    ...
    def verify_transaction(self, transaction):
        ...
        if transaction["amount"] == 0 and (
                (transaction["nft_data"] == "") == (transaction["nft_origin"] == "") or 10000 < len(
                transaction["nft_data"])):
            return False
    ...
    def verify_chain(self, chain):
        ...
        for i in range(len(chain["blocks"])):
            ...
            if i == 0:
                ...
                if format(int(self.hash(block_without_time), 16), "0256b")[
                   -current_pow_difficulty:] != '0' * current_pow_difficulty:
                    return False
    ...
    def create_new_block(self, miner):
        ...
        while not format(int(self.hash(block_without_time), 16), "0256b")[
                  -self.current_pow_difficulty:] == '0' * self.current_pow_difficulty:
        ...
        with ThreadPoolExecutor() as executor:
            for url in node_list.Node_List:
                if url != self.my_address:
                    executor.submit(requests.post, "http://" + url + ":8000/receive_transaction",
    ...
    def get_pow_difficulty(self, blocks, current_pow_difficulty):
        ...
        if (ix - 1) % POW_CHANGE_BLOCK_NUM == 0 and 1 < ix:
            ...
            for i in range(POW_CHANGE_BLOCK_NUM):
                all_time += (datetime.fromisoformat(blocks[ix - i]["time"]) - datetime.fromisoformat(
                    blocks[ix - i - 1]["time"])).total_seconds()
    ...
    def nft_calc(self, transactions):
        ...
        for transaction in transactions_copy:
            if transaction["amount"] == 0:
                ...
                elif nft_holder.get(transaction["nft_origin"]) == transaction["sender"] and transaction[
                    "nft_data"] == "":
